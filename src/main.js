import { readdirSync, existsSync, rmdirSync, mkdirSync, writeFileSync, stat } from "fs";
import pkg from 'canvas';
const { createCanvas, loadImage } = pkg;
import { log } from "console";
import { layersOrder, format, rarity } from"./config.js";
...
const saveLayer = (_canvas, _edition) => {
  writeFileSync(`${buildDir}/${_edition}.png`, _canvas.toBuffer("image/png"));
};
...
const createFiles = async edition => {
 ...
 for (let i = 1; i <= edition; i++) {
   ...
   if (Exists.has(key)) {
     ...
     Exists.set(key, i);
     addMetadata(i);
     log("Creating edition " + i);
   }
 }
};

const createMetaData = () => {
  stat(`${buildDir}/${metDataFile}`, (err) => {
    if(err == null || err.code === 'ENOENT') {
      writeFileSync(`${buildDir}/${metDataFile}`, JSON.stringify(metadata, null, 2));
    } else {
